...
public class AuthenticationService {
    ...
    public String login(Chef chef) {
        List<Chef> chief = chefService.searchChefs(chef.getUsername());
        Optional<Chef> matchingChef = null;
        
        for(Chef c : chief) {
            if(c.getUsername().equals(chef.getUsername()) && c.getPassword().equals(chef.getPassword())) {
                matchingChef = Optional.of(c);
                break;
            }
        }

        if(matchingChef.isPresent()) {
            String token = UUID.randomUUID().toString();
            loggedInUsers.put(token, matchingChef.get());
            return token;
        }
